...
{
public:
    ha_register(curl::Context& context, haconfig const& haconfig, token_counter& tc, std::size_t numnames, std::size_t const maxconns)
    : c_( context )
    , haconfig_( haconfig )
    , tc_( tc )
    , maxconns_( maxconns )
...
void register_names(InputIterator first, InputIterator const last, curl::Context& context, token_counter& tc, std::size_t numnames, std::size_t const maxconn)
{
    while ( first != last )
    {
        if ( first->status == GOOD )
        {
            boost::shared_ptr< ha_register > r( new ha_register(context, *first, tc, numnames, maxconn) );
            r->async_run();
        }
        ++first;
    }
