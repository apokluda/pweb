	...
	if (r > 0)
	{
		...
		for (k = m + 1, i = 2; i <= r; ++i)
		{
			...
			for (ksubset_lex_unrank(rm->s, i, 0, rm->subset[1]), flg = TRUE;
					flg;
					flg = ksubset_lex_succ(rm->s, i, rm->subset[0],
							rm->subset[1]))
			{
	...
	for (i = rm->r, pos = rm->k - 1; i >= 0; --i)
	{
		for (j = rm->s->nCr[rm->m][i] - 1; j >= 0; --j)
		{
			...
			for (k = 0; k < bound1; ++k)
			{
				...
				{
					if (k & (1 << l))
						vector_copy(rm->n, rm->rows[rm->subset[1][l]],
								rm->vector[1]);
					else
						vector_not(rm->q, rm->n, rm->rows[rm->subset[1][l]],
								rm->vector[1]);
					vector_multiply(rm->q, rm->n, rm->vector[0], rm->vector[1],
							rm->vector[2]);
				...
				if (vector_dotproduct(rm->q, rm->n, rm->vector[3],
						rm->vector[0]))
					++coeff1;
				else
					++coeff0;
		...
		vector_subtract(rm->q, rm->n, rm->vector[3], rm->vector[0],
				rm->vector[1]);
